...
def train(current_host, hosts, num_cpus, num_gpus, training_dir, model_dir,
    ...
    for epoch in range(epochs):
        # reset data iterator and metrics at begining of epoch.
        accuracy.reset()
        btic = time.time()
        i = 0
        for batch in train_iterator:
            ...
            with autograd.record():
                output = net(data)
                obtained_loss = loss(output, label)
        ...
        acc_tuple, f1_tuple, val_precision, val_recall = test(ctx, net, val_iterator)
        name, val_acc = acc_tuple
        print('[Epoch %d] Validation: %s=%f' % (epoch, name, val_acc))

        name, val_f1 = f1_tuple
        print(f'[Epoch {epoch}] Validation: {name}={val_f1}')

        print(f'[Epoch {epoch}] Validation: Precision={val_precision}')

        print(f'[Epoch {epoch}] Validation: Recall={val_recall}')

        if checkpoints_enabled:
            if val_acc > best_acc_score:
                best_acc_score = val_acc
                logging.info(f'Epoch: {epoch} - Saving the model, params and optimizer state with accuracy: {val_acc}.')
                ...
                logging.info(
                    f'Epoch: {epoch} - Did not save a model with a lower accuracy ({val_acc}) than the current best ({best_acc_score}).')
        else:
            print(f'Epoch: {epoch} - No checkpointing, but accuracy is: {val_acc}.')
...
def test(ctx, net, val_data):
    ...
    for batch in val_data:
        ...
        output = net(data)
        accuracy.update([label], [output])
        f1.update([label], [output])
        local_metrics.update([label], [output])
    return accuracy.get(), f1.get(), local_metrics.precision(), local_metrics.recall()
    ...
    def update(self, labels, preds):
        for label, pred in zip(labels, preds):
            ...
            if len(np.unique(label)) > 2:
                raise ValueError(f'{self.__class__.__name__} currently only supports binary classification.')
...
if __name__ == '__main__':
    args = parse_args()
    sm_num_cpus = int(os.environ['SM_NUM_CPUS'])
    sm_num_gpus = int(os.environ['SM_NUM_GPUS'])

    model = train(args.current_host, args.hosts, sm_num_cpus, sm_num_gpus, args.training_channel, args.model_dir,
...
def transform_fn(net, data, input_content_type, output_content_type):
    ...
    for row in parsed:
        ...
        result = [row, int(prediction.asscalar()), str(output[0][prediction].asscalar())]
