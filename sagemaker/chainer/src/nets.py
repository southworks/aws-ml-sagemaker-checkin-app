...
class TextClassifier(chainer.Chain):
    ...
    def __call__(self, xs, ys):
        ...
        f1 = F.f1_score(concat_outputs, concat_truths)
        precision = F.precision(concat_outputs, concat_truths)
        recall = F.recall(concat_outputs, concat_truths)
        reporter.report({'loss': loss.data}, self)
        reporter.report({'accuracy': accuracy.data}, self)
        
        reporter.report({'f1': f1[0]}, self)
        reporter.report({'precision': precision[0]}, self)
        reporter.report({'recall': recall[0]}, self)
